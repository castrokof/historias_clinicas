...
@section('contenido')
@include('admin.parametros.especialidades.tablas.tablaIndexEspecialidades')
@include('admin.parametros.especialidades.modal.modalEspecialidades')
@endsection
...
<script>
  $(document).ready(function() {
    ...
    $('#create_especialidad').click(function() {
      ...
      $('.card-title').text('Agregar Nueva Especialidad');
    ...
    $('#form-general').on('submit', function(event) {
      ...
      if ($('#action').val() == 'Add') {
        text = "Estás por crear un Especialidad"
        url = "{{route('guardar_especialidades')}}";
        method = 'post';
      }

      if ($('#action').val() == 'Edit') {
        text = "Estás por actualizar un Especialidad"
        var updateid = $('#hidden_id').val();
        url = "/especialidades/" + updateid;
        method = 'put';
      }
      Swal.fire({
        title: "¿Estás seguro?",
          ...
          $.ajax({
            ...
            success: function(data) {
              ...
              if (data.success == 'ok') {
                $('#form-general')[0].reset();
                $('#modal-u').modal('hide');
                $('#tespecialidad').DataTable().ajax.reload();
                Swal.fire({
                  icon: 'success',
    ...
    $(document).on('click', '.edit', function() {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          $('#hidden_id').val(id);
          $('.card-title').text('Editar Especialidad');
